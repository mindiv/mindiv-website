    ...
    <div className="flex justify-center items-center h-screen">
      <div className="flex flex-col w-2/3">
        <div className=" mb-10 relative flex justify-center">
          ...
          <div className="bg-white p-10 text-center rounded-lg  shadow_g relative z-3 w-full font-bold flex  flex-col justify-center items-center">
            <p className="text-lg mb-10">{question}</p>
            <div className="grid grid-cols-2 gap-4 w-full">
              {options.map((option, index) => (
                <OptionButton index={index}>{option}</OptionButton>
              ))}
            </div>
          </div>
        </div>
